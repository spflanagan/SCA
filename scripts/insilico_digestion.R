...
for(i in 1:length(mut.files))
{
  ...
  if(m == "u10E7") { mut<-0.0000001 }
  if(m == "u10E8") { mut<-0.00000001}
  theta<-4*as.numeric(n)*as.numeric(mut)
  mut.dat<-data.frame(rbind(mut.dat,
    cbind(Ne=rep(as.numeric(n),length(fsts)),MinMut=rep(as.numeric(mut),length(fsts)),
          theta = rep(as.numeric(theta),length(fsts)),Fst=as.numeric(fsts))),stringsAsFactors=FALSE)
}
...
###PLOTS####
#colors
is.col<-c('#8dd3c7','#ffffb3','#bebada','#fb8072','#80b1d3','#1b9e77','#d95f02','#7570b3','#e7298a','#66a61e')
names(is.col)<-c("s50","s80","s88","s57","s58","a50","a80","a88","a87","a58")

jpeg("insilico_results_revised.jpeg",height=8,width=10.5,units="in",res=300)
par(mfrow=c(2,2),oma=c(9.5,2,0.5,2),mar=c(2,1,2,2))
boxplot(pcr.afs.dat$Fst~pcr.afs.dat$AFS*pcr.afs.dat$PCR,
  col=c(is.col["s50"],is.col["s80"]),pch=21,axes=F,ylim=c(0,0.02),
  outcol="black",outbg=c(is.col["s50"],is.col["s80"]))
...
axis(2,pos=0.5,las=1,at=seq(0,0.05,0.01))
...
boxplot(mut.dat$Fst~mut.dat$MinMut*as.numeric(as.character(mut.dat$Ne)),
  pch=21,axes=F,ylim=c(0,0.02),col=c(is.col["s57"],is.col["s58"]),
  outcol="black",outbg=c(is.col["s57"],is.col["s58"]))
axis(1,at=seq(-1.5,8.5,2),labels=rep("",6),pos=-0.0008)
axis(1,at=seq(1.5,5.5,2),labels=c(5000,10000,20000),pos=0,tick=F,tck=0)
axis(2,pos=0.5,las=1,at=seq(-0.01,0.03,0.01))
mtext(expression(italic(N)[italic(e)]),1,outer=F,line=2,cex=0.75)

boxplot(null.dat$Fst~factor(null.dat$Asymmetric)*factor(null.dat$Skew)*factor(null.dat$Shearing),
        col=c(is.col["a50"],is.col["s50"],is.col["a80"],is.col["s80"]),
        pch=c(rep(21,4),rep(23,4)),axes=F,ylim=c(0,0.02),
        outcol="black",outbg=c(is.col["a50"],is.col["s50"],is.col["a80"],is.col["s80"]))
...
boxplot(comb.dat$Fst~comb.dat$Description,axes=F,col=c(is.col["a87"],is.col["a58"],is.col["a88"],is.col["a88"],is.col["s88"]),
        outbg=c(is.col["a87"],is.col["a58"],is.col["a88"],is.col["a88"],is.col["s88"]),pch=c(23,23,23,21,23),ylim=c(0,0.02))
axis(1,at=c(-0.5,0.5,5.5,10.5,15.5,17),labels=rep("",6),pos=-0.0008)
axis(1,at=c(3,8,13),pos=-0.001,tick=F,tck=0,
     labels=c("1% of reads from\nPCR duplicates","2% of reads from\nPCR duplicates","3% of reads from\nPCR duplicates"))
axis(2,pos=0.5,las=1,at=seq(-0.01,0.04,0.01))
mtext(expression(italic(F)[ST]),2,outer=T,cex=0.75)

#add a legend
par(fig = c(0, 1, 0, 1), oma=c(2,1,0,1), mar = c(0, 0, 0, 0), new = TRUE, cex=1)
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
legend("bottom",c("symmetric sampling, mean p = 0.5, no mutation",#s50
               "symmetric sampling, mean p = 0.8, no mutation",#s80
               expression("symmetric sampling, mean p = 0.8,"~mu~"from ["~10^{-9}~","~10^{-8}~"]"),#s88
               expression("symmetric sampling, mean p = 0.8,"~mu~"from ["~10^{-8}~","~10^{-7}~"]"),#s87
               expression("symmetric sampling, mean p = 0.5,"~mu~"from ["~10^{-9}~","~10^{-8}~"]"),#s58
               "asymmetric sampling, mean p = 0.5, no mutation",#a50
               "asymmetric sampling, mean p = 0.8, no mutation",#a80
               expression("asymmetric sampling, mean p = 0.8,"~mu~"from ["~10^{-9}~","~10^{-8}~"]"),#a88
               expression("asymmetric sampling, mean p = 0.8,"~mu~"from ["~10^{-8}~","~10^{-7}~"]"),#a87
               expression("asymmetric sampling, mean p = 0.5,"~mu~"from ["~10^{-9}~","~10^{-8}~"]")),#a58
       fill=is.col,border=is.col,bty='n',ncol=2,cex=0.85,inset=c(0,0.01))
legend("bottom",c("shearing bias", "no shearing bias"),ncol=2,bty='n',pch=c(23,21),inset=c(0,-0.01),cex=0.85)
dev.off()

##STATS##
summary(aov(pcr.afs.dat$Fst~pcr.afs.dat$AFS*pcr.afs.dat$PCR))
summary(aov(mut.dat$Fst~mut.dat$theta))
summary(aov(mut.dat$Fst~mut.dat$Ne*mut.dat$MinMut))
summary(aov(null.dat$Fst~factor(null.dat$Asymmetric)*factor(null.dat$Skew)*factor(null.dat$Shearing)))
